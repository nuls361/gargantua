...
def main():
    ...
    print(f"checking {len(handles)} unreconciled creators against Attio…", flush=True)

    matched = 0
    for i in range(0, len(handles), args.batch):
        batch = handles[i:i + args.batch]
        try:
            found = query_attio(batch, key)
        except requests.HTTPError as e:
            print(f"  ! Attio error on batch {i}: {e}", flush=True)
            continue
        for h in batch:
            sec = sec_by_handle[h]
            rec = found.get(h)
        ...
        print(f"  … {min(i + args.batch, len(handles))}/{len(handles)}  ({matched} Songpush so far)", flush=True)
        time.sleep(0.3)

    print(f"done — {matched}/{len(handles)} are already Songpush users.", flush=True)
