// Server-side copy of the freemail cleaning logic (mirrors src/lib/filter.ts).
// Used by enrich-list to keep only private freemail addresses.

export const FREEMAIL_ALLOWLIST = new Set<string>([
  // Global majors
...
// Returns null if the email passes cleaning, otherwise the reason it fails.
// (Duplicate-email detection is done separately in enrich-list against the DB.)
export function cleanReason(
  ...
  if (blockedDomains.has(domain)) return "blocked_domain";
  if (!FREEMAIL_ALLOWLIST.has(domain)) return "not_freemail";
  return null;
}
