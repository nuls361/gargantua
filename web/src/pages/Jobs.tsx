...
function JobDetail({ id }: { id: string }) {
  ...
  const [sortKey, setSortKey] = useState("vibe");
  ...
  const sorted = [...members].sort((a, b) => {
    const av = sortKey === "vibe" ? a.similarity : Number((a as Record<string, unknown>)[sortKey] ?? 0);
    const bv = sortKey === "vibe" ? b.similarity : Number((b as Record<string, unknown>)[sortKey] ?? 0);
            ...
            return (
              <div key={m.sec_uid} className={"crow" + (m.is_songpush_user ? " wepush" : "")} onClick={() => setPanel(m)}>
                ...
                <div className="metrics">
                  ...
                  {m.similarity >= 0 && <div className="metric"><div className="v num" style={{ color: "var(--wp-acc)" }} title="Visual vibe match to the sample creators">{Math.round(m.similarity * 100)}</div><div className="k">Vibe</div></div>}
